...
public class VocabComparisonGroup extends BaseEntity implements Serializable {

    @Id
    @Column(name = "id")
    @ApiModelProperty(value = "词汇辨析组ID", hidden = true)
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @Column(name = "group_key", nullable = false, length = 64)
    @ApiModelProperty(value = "辨析组标识（如'标准vs尺度'）")
